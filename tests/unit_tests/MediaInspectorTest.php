...
class MediaInspectorTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testGetMediaInfoWithLocation() {
        ...
        $media_inspector = new Instagram\MediaInspector($rest_client_stub);

        $ACCESS_TOKEN = 'access.token.123';
        $MEDIA_ID = '1081347983064313090';

        $media_info = $media_inspector->getMediaInfo($ACCESS_TOKEN, $MEDIA_ID);
        $expected_location = new Utils\GeoPoint('40.7155418', '-73.9533691');
        $expected_media_info = new Domain\MediaInfo(
            '1081347983064313090_1220832186', 'image', $expected_location);

        $this->assertEquals($expected_media_info, $media_info);
    }

    public function testGetMediaInfoWithOutLocation() {
        $rest_client_stub = $this->getMockBuilder('\PhotoInspector\RESTAdapeter\RESTClient')
                                 ->setMethods(array('get'))
                                 ->getMock();
        ...
        $media_info = $media_inspector->getMediaInfo($ACCESS_TOKEN, $MEDIA_ID);
        $expected_media_info = new Domain\MediaInfo('420077066_4190444', 'image');
